// Code generated by protoc-gen-go. DO NOT EDIT.
// versions:
// 	protoc-gen-go v1.36.6
// 	protoc        v5.29.3
// source: studio.proto

package generated

import (
	protoreflect "google.golang.org/protobuf/reflect/protoreflect"
	protoimpl "google.golang.org/protobuf/runtime/protoimpl"
	fieldmaskpb "google.golang.org/protobuf/types/known/fieldmaskpb"
	timestamppb "google.golang.org/protobuf/types/known/timestamppb"
	reflect "reflect"
	sync "sync"
	unsafe "unsafe"
)

const (
	// Verify that this generated code is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(20 - protoimpl.MinVersion)
	// Verify that runtime/protoimpl is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(protoimpl.MaxVersion - 20)
)

type CreateStudioRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Studio        *Studio                `protobuf:"bytes,1,opt,name=studio,proto3" json:"studio,omitempty"`
	Request       *BaseRequest           `protobuf:"bytes,100,opt,name=request,proto3" json:"request,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *CreateStudioRequest) Reset() {
	*x = CreateStudioRequest{}
	mi := &file_studio_proto_msgTypes[0]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *CreateStudioRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*CreateStudioRequest) ProtoMessage() {}

func (x *CreateStudioRequest) ProtoReflect() protoreflect.Message {
	mi := &file_studio_proto_msgTypes[0]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use CreateStudioRequest.ProtoReflect.Descriptor instead.
func (*CreateStudioRequest) Descriptor() ([]byte, []int) {
	return file_studio_proto_rawDescGZIP(), []int{0}
}

func (x *CreateStudioRequest) GetStudio() *Studio {
	if x != nil {
		return x.Studio
	}
	return nil
}

func (x *CreateStudioRequest) GetRequest() *BaseRequest {
	if x != nil {
		return x.Request
	}
	return nil
}

type CreateStudioResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	StudioId      string                 `protobuf:"bytes,1,opt,name=studio_id,json=studioId,proto3" json:"studio_id,omitempty"`
	Message       string                 `protobuf:"bytes,2,opt,name=message,proto3" json:"message,omitempty"`
	Response      *BaseResponse          `protobuf:"bytes,100,opt,name=response,proto3" json:"response,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *CreateStudioResponse) Reset() {
	*x = CreateStudioResponse{}
	mi := &file_studio_proto_msgTypes[1]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *CreateStudioResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*CreateStudioResponse) ProtoMessage() {}

func (x *CreateStudioResponse) ProtoReflect() protoreflect.Message {
	mi := &file_studio_proto_msgTypes[1]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use CreateStudioResponse.ProtoReflect.Descriptor instead.
func (*CreateStudioResponse) Descriptor() ([]byte, []int) {
	return file_studio_proto_rawDescGZIP(), []int{1}
}

func (x *CreateStudioResponse) GetStudioId() string {
	if x != nil {
		return x.StudioId
	}
	return ""
}

func (x *CreateStudioResponse) GetMessage() string {
	if x != nil {
		return x.Message
	}
	return ""
}

func (x *CreateStudioResponse) GetResponse() *BaseResponse {
	if x != nil {
		return x.Response
	}
	return nil
}

type UpdateStudioRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	StudioId      string                 `protobuf:"bytes,1,opt,name=studio_id,json=studioId,proto3" json:"studio_id,omitempty"`
	Studio        *Studio                `protobuf:"bytes,2,opt,name=studio,proto3" json:"studio,omitempty"`
	UpdateMask    *fieldmaskpb.FieldMask `protobuf:"bytes,3,opt,name=update_mask,json=updateMask,proto3" json:"update_mask,omitempty"`
	Base          *BaseRequest           `protobuf:"bytes,100,opt,name=base,proto3" json:"base,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *UpdateStudioRequest) Reset() {
	*x = UpdateStudioRequest{}
	mi := &file_studio_proto_msgTypes[2]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *UpdateStudioRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*UpdateStudioRequest) ProtoMessage() {}

func (x *UpdateStudioRequest) ProtoReflect() protoreflect.Message {
	mi := &file_studio_proto_msgTypes[2]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use UpdateStudioRequest.ProtoReflect.Descriptor instead.
func (*UpdateStudioRequest) Descriptor() ([]byte, []int) {
	return file_studio_proto_rawDescGZIP(), []int{2}
}

func (x *UpdateStudioRequest) GetStudioId() string {
	if x != nil {
		return x.StudioId
	}
	return ""
}

func (x *UpdateStudioRequest) GetStudio() *Studio {
	if x != nil {
		return x.Studio
	}
	return nil
}

func (x *UpdateStudioRequest) GetUpdateMask() *fieldmaskpb.FieldMask {
	if x != nil {
		return x.UpdateMask
	}
	return nil
}

func (x *UpdateStudioRequest) GetBase() *BaseRequest {
	if x != nil {
		return x.Base
	}
	return nil
}

type UpdateStudioResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Success       bool                   `protobuf:"varint,1,opt,name=success,proto3" json:"success,omitempty"`
	Message       string                 `protobuf:"bytes,2,opt,name=message,proto3" json:"message,omitempty"`
	Studio        *Studio                `protobuf:"bytes,3,opt,name=studio,proto3" json:"studio,omitempty"`
	Response      *BaseResponse          `protobuf:"bytes,100,opt,name=response,proto3" json:"response,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *UpdateStudioResponse) Reset() {
	*x = UpdateStudioResponse{}
	mi := &file_studio_proto_msgTypes[3]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *UpdateStudioResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*UpdateStudioResponse) ProtoMessage() {}

func (x *UpdateStudioResponse) ProtoReflect() protoreflect.Message {
	mi := &file_studio_proto_msgTypes[3]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use UpdateStudioResponse.ProtoReflect.Descriptor instead.
func (*UpdateStudioResponse) Descriptor() ([]byte, []int) {
	return file_studio_proto_rawDescGZIP(), []int{3}
}

func (x *UpdateStudioResponse) GetSuccess() bool {
	if x != nil {
		return x.Success
	}
	return false
}

func (x *UpdateStudioResponse) GetMessage() string {
	if x != nil {
		return x.Message
	}
	return ""
}

func (x *UpdateStudioResponse) GetStudio() *Studio {
	if x != nil {
		return x.Studio
	}
	return nil
}

func (x *UpdateStudioResponse) GetResponse() *BaseResponse {
	if x != nil {
		return x.Response
	}
	return nil
}

type AddStaffMemberRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	StudioId      string                 `protobuf:"bytes,1,opt,name=studio_id,json=studioId,proto3" json:"studio_id,omitempty"`
	UserId        string                 `protobuf:"bytes,2,opt,name=user_id,json=userId,proto3" json:"user_id,omitempty"`
	Role          string                 `protobuf:"bytes,3,opt,name=role,proto3" json:"role,omitempty"`
	Permissions   []string               `protobuf:"bytes,4,rep,name=permissions,proto3" json:"permissions,omitempty"`
	Request       *BaseRequest           `protobuf:"bytes,100,opt,name=request,proto3" json:"request,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *AddStaffMemberRequest) Reset() {
	*x = AddStaffMemberRequest{}
	mi := &file_studio_proto_msgTypes[4]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *AddStaffMemberRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*AddStaffMemberRequest) ProtoMessage() {}

func (x *AddStaffMemberRequest) ProtoReflect() protoreflect.Message {
	mi := &file_studio_proto_msgTypes[4]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use AddStaffMemberRequest.ProtoReflect.Descriptor instead.
func (*AddStaffMemberRequest) Descriptor() ([]byte, []int) {
	return file_studio_proto_rawDescGZIP(), []int{4}
}

func (x *AddStaffMemberRequest) GetStudioId() string {
	if x != nil {
		return x.StudioId
	}
	return ""
}

func (x *AddStaffMemberRequest) GetUserId() string {
	if x != nil {
		return x.UserId
	}
	return ""
}

func (x *AddStaffMemberRequest) GetRole() string {
	if x != nil {
		return x.Role
	}
	return ""
}

func (x *AddStaffMemberRequest) GetPermissions() []string {
	if x != nil {
		return x.Permissions
	}
	return nil
}

func (x *AddStaffMemberRequest) GetRequest() *BaseRequest {
	if x != nil {
		return x.Request
	}
	return nil
}

type AddStaffMemberResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	StaffId       string                 `protobuf:"bytes,1,opt,name=staff_id,json=staffId,proto3" json:"staff_id,omitempty"`
	Message       string                 `protobuf:"bytes,2,opt,name=message,proto3" json:"message,omitempty"`
	Response      *BaseResponse          `protobuf:"bytes,100,opt,name=response,proto3" json:"response,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *AddStaffMemberResponse) Reset() {
	*x = AddStaffMemberResponse{}
	mi := &file_studio_proto_msgTypes[5]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *AddStaffMemberResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*AddStaffMemberResponse) ProtoMessage() {}

func (x *AddStaffMemberResponse) ProtoReflect() protoreflect.Message {
	mi := &file_studio_proto_msgTypes[5]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use AddStaffMemberResponse.ProtoReflect.Descriptor instead.
func (*AddStaffMemberResponse) Descriptor() ([]byte, []int) {
	return file_studio_proto_rawDescGZIP(), []int{5}
}

func (x *AddStaffMemberResponse) GetStaffId() string {
	if x != nil {
		return x.StaffId
	}
	return ""
}

func (x *AddStaffMemberResponse) GetMessage() string {
	if x != nil {
		return x.Message
	}
	return ""
}

func (x *AddStaffMemberResponse) GetResponse() *BaseResponse {
	if x != nil {
		return x.Response
	}
	return nil
}

type UpdateStaffMemberRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	StaffId       string                 `protobuf:"bytes,1,opt,name=staff_id,json=staffId,proto3" json:"staff_id,omitempty"`
	Role          string                 `protobuf:"bytes,2,opt,name=role,proto3" json:"role,omitempty"`
	Request       *BaseRequest           `protobuf:"bytes,100,opt,name=request,proto3" json:"request,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *UpdateStaffMemberRequest) Reset() {
	*x = UpdateStaffMemberRequest{}
	mi := &file_studio_proto_msgTypes[6]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *UpdateStaffMemberRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*UpdateStaffMemberRequest) ProtoMessage() {}

func (x *UpdateStaffMemberRequest) ProtoReflect() protoreflect.Message {
	mi := &file_studio_proto_msgTypes[6]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use UpdateStaffMemberRequest.ProtoReflect.Descriptor instead.
func (*UpdateStaffMemberRequest) Descriptor() ([]byte, []int) {
	return file_studio_proto_rawDescGZIP(), []int{6}
}

func (x *UpdateStaffMemberRequest) GetStaffId() string {
	if x != nil {
		return x.StaffId
	}
	return ""
}

func (x *UpdateStaffMemberRequest) GetRole() string {
	if x != nil {
		return x.Role
	}
	return ""
}

func (x *UpdateStaffMemberRequest) GetRequest() *BaseRequest {
	if x != nil {
		return x.Request
	}
	return nil
}

type UpdateStaffMemberResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Message       string                 `protobuf:"bytes,1,opt,name=message,proto3" json:"message,omitempty"`
	Response      *BaseResponse          `protobuf:"bytes,100,opt,name=response,proto3" json:"response,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *UpdateStaffMemberResponse) Reset() {
	*x = UpdateStaffMemberResponse{}
	mi := &file_studio_proto_msgTypes[7]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *UpdateStaffMemberResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*UpdateStaffMemberResponse) ProtoMessage() {}

func (x *UpdateStaffMemberResponse) ProtoReflect() protoreflect.Message {
	mi := &file_studio_proto_msgTypes[7]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use UpdateStaffMemberResponse.ProtoReflect.Descriptor instead.
func (*UpdateStaffMemberResponse) Descriptor() ([]byte, []int) {
	return file_studio_proto_rawDescGZIP(), []int{7}
}

func (x *UpdateStaffMemberResponse) GetMessage() string {
	if x != nil {
		return x.Message
	}
	return ""
}

func (x *UpdateStaffMemberResponse) GetResponse() *BaseResponse {
	if x != nil {
		return x.Response
	}
	return nil
}

type RemoveStaffMemberRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	StaffId       string                 `protobuf:"bytes,1,opt,name=staff_id,json=staffId,proto3" json:"staff_id,omitempty"`
	Request       *BaseRequest           `protobuf:"bytes,100,opt,name=request,proto3" json:"request,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *RemoveStaffMemberRequest) Reset() {
	*x = RemoveStaffMemberRequest{}
	mi := &file_studio_proto_msgTypes[8]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *RemoveStaffMemberRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*RemoveStaffMemberRequest) ProtoMessage() {}

func (x *RemoveStaffMemberRequest) ProtoReflect() protoreflect.Message {
	mi := &file_studio_proto_msgTypes[8]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use RemoveStaffMemberRequest.ProtoReflect.Descriptor instead.
func (*RemoveStaffMemberRequest) Descriptor() ([]byte, []int) {
	return file_studio_proto_rawDescGZIP(), []int{8}
}

func (x *RemoveStaffMemberRequest) GetStaffId() string {
	if x != nil {
		return x.StaffId
	}
	return ""
}

func (x *RemoveStaffMemberRequest) GetRequest() *BaseRequest {
	if x != nil {
		return x.Request
	}
	return nil
}

type RemoveStaffMemberResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Message       string                 `protobuf:"bytes,1,opt,name=message,proto3" json:"message,omitempty"`
	Response      *BaseResponse          `protobuf:"bytes,100,opt,name=response,proto3" json:"response,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *RemoveStaffMemberResponse) Reset() {
	*x = RemoveStaffMemberResponse{}
	mi := &file_studio_proto_msgTypes[9]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *RemoveStaffMemberResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*RemoveStaffMemberResponse) ProtoMessage() {}

func (x *RemoveStaffMemberResponse) ProtoReflect() protoreflect.Message {
	mi := &file_studio_proto_msgTypes[9]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use RemoveStaffMemberResponse.ProtoReflect.Descriptor instead.
func (*RemoveStaffMemberResponse) Descriptor() ([]byte, []int) {
	return file_studio_proto_rawDescGZIP(), []int{9}
}

func (x *RemoveStaffMemberResponse) GetMessage() string {
	if x != nil {
		return x.Message
	}
	return ""
}

func (x *RemoveStaffMemberResponse) GetResponse() *BaseResponse {
	if x != nil {
		return x.Response
	}
	return nil
}

type ListStaffMembersRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	StudioId      string                 `protobuf:"bytes,1,opt,name=studio_id,json=studioId,proto3" json:"studio_id,omitempty"`
	Request       *BaseRequest           `protobuf:"bytes,100,opt,name=request,proto3" json:"request,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ListStaffMembersRequest) Reset() {
	*x = ListStaffMembersRequest{}
	mi := &file_studio_proto_msgTypes[10]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ListStaffMembersRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListStaffMembersRequest) ProtoMessage() {}

func (x *ListStaffMembersRequest) ProtoReflect() protoreflect.Message {
	mi := &file_studio_proto_msgTypes[10]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListStaffMembersRequest.ProtoReflect.Descriptor instead.
func (*ListStaffMembersRequest) Descriptor() ([]byte, []int) {
	return file_studio_proto_rawDescGZIP(), []int{10}
}

func (x *ListStaffMembersRequest) GetStudioId() string {
	if x != nil {
		return x.StudioId
	}
	return ""
}

func (x *ListStaffMembersRequest) GetRequest() *BaseRequest {
	if x != nil {
		return x.Request
	}
	return nil
}

type ListStaffMembersResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	StaffMembers  []*StaffMember         `protobuf:"bytes,1,rep,name=staff_members,json=staffMembers,proto3" json:"staff_members,omitempty"`
	Response      *BaseResponse          `protobuf:"bytes,100,opt,name=response,proto3" json:"response,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ListStaffMembersResponse) Reset() {
	*x = ListStaffMembersResponse{}
	mi := &file_studio_proto_msgTypes[11]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ListStaffMembersResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListStaffMembersResponse) ProtoMessage() {}

func (x *ListStaffMembersResponse) ProtoReflect() protoreflect.Message {
	mi := &file_studio_proto_msgTypes[11]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListStaffMembersResponse.ProtoReflect.Descriptor instead.
func (*ListStaffMembersResponse) Descriptor() ([]byte, []int) {
	return file_studio_proto_rawDescGZIP(), []int{11}
}

func (x *ListStaffMembersResponse) GetStaffMembers() []*StaffMember {
	if x != nil {
		return x.StaffMembers
	}
	return nil
}

func (x *ListStaffMembersResponse) GetResponse() *BaseResponse {
	if x != nil {
		return x.Response
	}
	return nil
}

type SetStaffPermissionsRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	StaffId       string                 `protobuf:"bytes,1,opt,name=staff_id,json=staffId,proto3" json:"staff_id,omitempty"`
	Permissions   []string               `protobuf:"bytes,2,rep,name=permissions,proto3" json:"permissions,omitempty"`
	Request       *BaseRequest           `protobuf:"bytes,100,opt,name=request,proto3" json:"request,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *SetStaffPermissionsRequest) Reset() {
	*x = SetStaffPermissionsRequest{}
	mi := &file_studio_proto_msgTypes[12]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *SetStaffPermissionsRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*SetStaffPermissionsRequest) ProtoMessage() {}

func (x *SetStaffPermissionsRequest) ProtoReflect() protoreflect.Message {
	mi := &file_studio_proto_msgTypes[12]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use SetStaffPermissionsRequest.ProtoReflect.Descriptor instead.
func (*SetStaffPermissionsRequest) Descriptor() ([]byte, []int) {
	return file_studio_proto_rawDescGZIP(), []int{12}
}

func (x *SetStaffPermissionsRequest) GetStaffId() string {
	if x != nil {
		return x.StaffId
	}
	return ""
}

func (x *SetStaffPermissionsRequest) GetPermissions() []string {
	if x != nil {
		return x.Permissions
	}
	return nil
}

func (x *SetStaffPermissionsRequest) GetRequest() *BaseRequest {
	if x != nil {
		return x.Request
	}
	return nil
}

type SetStaffPermissionsResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Message       string                 `protobuf:"bytes,1,opt,name=message,proto3" json:"message,omitempty"`
	Response      *BaseResponse          `protobuf:"bytes,100,opt,name=response,proto3" json:"response,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *SetStaffPermissionsResponse) Reset() {
	*x = SetStaffPermissionsResponse{}
	mi := &file_studio_proto_msgTypes[13]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *SetStaffPermissionsResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*SetStaffPermissionsResponse) ProtoMessage() {}

func (x *SetStaffPermissionsResponse) ProtoReflect() protoreflect.Message {
	mi := &file_studio_proto_msgTypes[13]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use SetStaffPermissionsResponse.ProtoReflect.Descriptor instead.
func (*SetStaffPermissionsResponse) Descriptor() ([]byte, []int) {
	return file_studio_proto_rawDescGZIP(), []int{13}
}

func (x *SetStaffPermissionsResponse) GetMessage() string {
	if x != nil {
		return x.Message
	}
	return ""
}

func (x *SetStaffPermissionsResponse) GetResponse() *BaseResponse {
	if x != nil {
		return x.Response
	}
	return nil
}

type GetStaffPermissionsRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	StaffId       string                 `protobuf:"bytes,1,opt,name=staff_id,json=staffId,proto3" json:"staff_id,omitempty"`
	Request       *BaseRequest           `protobuf:"bytes,100,opt,name=request,proto3" json:"request,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetStaffPermissionsRequest) Reset() {
	*x = GetStaffPermissionsRequest{}
	mi := &file_studio_proto_msgTypes[14]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetStaffPermissionsRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetStaffPermissionsRequest) ProtoMessage() {}

func (x *GetStaffPermissionsRequest) ProtoReflect() protoreflect.Message {
	mi := &file_studio_proto_msgTypes[14]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetStaffPermissionsRequest.ProtoReflect.Descriptor instead.
func (*GetStaffPermissionsRequest) Descriptor() ([]byte, []int) {
	return file_studio_proto_rawDescGZIP(), []int{14}
}

func (x *GetStaffPermissionsRequest) GetStaffId() string {
	if x != nil {
		return x.StaffId
	}
	return ""
}

func (x *GetStaffPermissionsRequest) GetRequest() *BaseRequest {
	if x != nil {
		return x.Request
	}
	return nil
}

type GetStaffPermissionsResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Permissions   []string               `protobuf:"bytes,1,rep,name=permissions,proto3" json:"permissions,omitempty"`
	Response      *BaseResponse          `protobuf:"bytes,100,opt,name=response,proto3" json:"response,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetStaffPermissionsResponse) Reset() {
	*x = GetStaffPermissionsResponse{}
	mi := &file_studio_proto_msgTypes[15]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetStaffPermissionsResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetStaffPermissionsResponse) ProtoMessage() {}

func (x *GetStaffPermissionsResponse) ProtoReflect() protoreflect.Message {
	mi := &file_studio_proto_msgTypes[15]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetStaffPermissionsResponse.ProtoReflect.Descriptor instead.
func (*GetStaffPermissionsResponse) Descriptor() ([]byte, []int) {
	return file_studio_proto_rawDescGZIP(), []int{15}
}

func (x *GetStaffPermissionsResponse) GetPermissions() []string {
	if x != nil {
		return x.Permissions
	}
	return nil
}

func (x *GetStaffPermissionsResponse) GetResponse() *BaseResponse {
	if x != nil {
		return x.Response
	}
	return nil
}

type Studio struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	StudioId      string                 `protobuf:"bytes,1,opt,name=studio_id,json=studioId,proto3" json:"studio_id,omitempty"`
	Name          string                 `protobuf:"bytes,2,opt,name=name,proto3" json:"name,omitempty"`
	Address       string                 `protobuf:"bytes,3,opt,name=address,proto3" json:"address,omitempty"`
	Phone         string                 `protobuf:"bytes,4,opt,name=phone,proto3" json:"phone,omitempty"`
	Email         string                 `protobuf:"bytes,5,opt,name=email,proto3" json:"email,omitempty"`
	Website       string                 `protobuf:"bytes,6,opt,name=website,proto3" json:"website,omitempty"`
	Owner         *OwnerInfo             `protobuf:"bytes,7,opt,name=owner,proto3" json:"owner,omitempty"`
	CreatedAt     *timestamppb.Timestamp `protobuf:"bytes,8,opt,name=created_at,json=createdAt,proto3" json:"created_at,omitempty"`
	UpdatedAt     *timestamppb.Timestamp `protobuf:"bytes,9,opt,name=updated_at,json=updatedAt,proto3" json:"updated_at,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *Studio) Reset() {
	*x = Studio{}
	mi := &file_studio_proto_msgTypes[16]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *Studio) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*Studio) ProtoMessage() {}

func (x *Studio) ProtoReflect() protoreflect.Message {
	mi := &file_studio_proto_msgTypes[16]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use Studio.ProtoReflect.Descriptor instead.
func (*Studio) Descriptor() ([]byte, []int) {
	return file_studio_proto_rawDescGZIP(), []int{16}
}

func (x *Studio) GetStudioId() string {
	if x != nil {
		return x.StudioId
	}
	return ""
}

func (x *Studio) GetName() string {
	if x != nil {
		return x.Name
	}
	return ""
}

func (x *Studio) GetAddress() string {
	if x != nil {
		return x.Address
	}
	return ""
}

func (x *Studio) GetPhone() string {
	if x != nil {
		return x.Phone
	}
	return ""
}

func (x *Studio) GetEmail() string {
	if x != nil {
		return x.Email
	}
	return ""
}

func (x *Studio) GetWebsite() string {
	if x != nil {
		return x.Website
	}
	return ""
}

func (x *Studio) GetOwner() *OwnerInfo {
	if x != nil {
		return x.Owner
	}
	return nil
}

func (x *Studio) GetCreatedAt() *timestamppb.Timestamp {
	if x != nil {
		return x.CreatedAt
	}
	return nil
}

func (x *Studio) GetUpdatedAt() *timestamppb.Timestamp {
	if x != nil {
		return x.UpdatedAt
	}
	return nil
}

type StaffMember struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	StaffId       string                 `protobuf:"bytes,1,opt,name=staff_id,json=staffId,proto3" json:"staff_id,omitempty"`
	StudioId      string                 `protobuf:"bytes,2,opt,name=studio_id,json=studioId,proto3" json:"studio_id,omitempty"`
	UserId        string                 `protobuf:"bytes,3,opt,name=user_id,json=userId,proto3" json:"user_id,omitempty"`
	Role          string                 `protobuf:"bytes,4,opt,name=role,proto3" json:"role,omitempty"`
	Permissions   []string               `protobuf:"bytes,5,rep,name=permissions,proto3" json:"permissions,omitempty"`
	CreatedAt     *timestamppb.Timestamp `protobuf:"bytes,6,opt,name=created_at,json=createdAt,proto3" json:"created_at,omitempty"`
	UpdatedAt     *timestamppb.Timestamp `protobuf:"bytes,7,opt,name=updated_at,json=updatedAt,proto3" json:"updated_at,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *StaffMember) Reset() {
	*x = StaffMember{}
	mi := &file_studio_proto_msgTypes[17]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *StaffMember) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*StaffMember) ProtoMessage() {}

func (x *StaffMember) ProtoReflect() protoreflect.Message {
	mi := &file_studio_proto_msgTypes[17]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use StaffMember.ProtoReflect.Descriptor instead.
func (*StaffMember) Descriptor() ([]byte, []int) {
	return file_studio_proto_rawDescGZIP(), []int{17}
}

func (x *StaffMember) GetStaffId() string {
	if x != nil {
		return x.StaffId
	}
	return ""
}

func (x *StaffMember) GetStudioId() string {
	if x != nil {
		return x.StudioId
	}
	return ""
}

func (x *StaffMember) GetUserId() string {
	if x != nil {
		return x.UserId
	}
	return ""
}

func (x *StaffMember) GetRole() string {
	if x != nil {
		return x.Role
	}
	return ""
}

func (x *StaffMember) GetPermissions() []string {
	if x != nil {
		return x.Permissions
	}
	return nil
}

func (x *StaffMember) GetCreatedAt() *timestamppb.Timestamp {
	if x != nil {
		return x.CreatedAt
	}
	return nil
}

func (x *StaffMember) GetUpdatedAt() *timestamppb.Timestamp {
	if x != nil {
		return x.UpdatedAt
	}
	return nil
}

type OwnerInfo struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Email         string                 `protobuf:"bytes,1,opt,name=email,proto3" json:"email,omitempty"`
	Password      string                 `protobuf:"bytes,2,opt,name=password,proto3" json:"password,omitempty"` // Hashed password
	DisplayName   string                 `protobuf:"bytes,3,opt,name=display_name,json=displayName,proto3" json:"display_name,omitempty"`
	Username      string                 `protobuf:"bytes,4,opt,name=username,proto3" json:"username,omitempty"`
	FirstName     string                 `protobuf:"bytes,5,opt,name=first_name,json=firstName,proto3" json:"first_name,omitempty"`
	LastName      string                 `protobuf:"bytes,6,opt,name=last_name,json=lastName,proto3" json:"last_name,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *OwnerInfo) Reset() {
	*x = OwnerInfo{}
	mi := &file_studio_proto_msgTypes[18]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *OwnerInfo) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*OwnerInfo) ProtoMessage() {}

func (x *OwnerInfo) ProtoReflect() protoreflect.Message {
	mi := &file_studio_proto_msgTypes[18]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use OwnerInfo.ProtoReflect.Descriptor instead.
func (*OwnerInfo) Descriptor() ([]byte, []int) {
	return file_studio_proto_rawDescGZIP(), []int{18}
}

func (x *OwnerInfo) GetEmail() string {
	if x != nil {
		return x.Email
	}
	return ""
}

func (x *OwnerInfo) GetPassword() string {
	if x != nil {
		return x.Password
	}
	return ""
}

func (x *OwnerInfo) GetDisplayName() string {
	if x != nil {
		return x.DisplayName
	}
	return ""
}

func (x *OwnerInfo) GetUsername() string {
	if x != nil {
		return x.Username
	}
	return ""
}

func (x *OwnerInfo) GetFirstName() string {
	if x != nil {
		return x.FirstName
	}
	return ""
}

func (x *OwnerInfo) GetLastName() string {
	if x != nil {
		return x.LastName
	}
	return ""
}

// For ListStudios RPC
type ListStudiosRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	PageSize      int32                  `protobuf:"varint,1,opt,name=page_size,json=pageSize,proto3" json:"page_size,omitempty"`
	PageNumber    int32                  `protobuf:"varint,2,opt,name=page_number,json=pageNumber,proto3" json:"page_number,omitempty"`
	Filter        string                 `protobuf:"bytes,3,opt,name=filter,proto3" json:"filter,omitempty"`
	OwnerId       string                 `protobuf:"bytes,4,opt,name=owner_id,json=ownerId,proto3" json:"owner_id,omitempty"` // Optional: to filter studios by owner
	Request       *BaseRequest           `protobuf:"bytes,100,opt,name=request,proto3" json:"request,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ListStudiosRequest) Reset() {
	*x = ListStudiosRequest{}
	mi := &file_studio_proto_msgTypes[19]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ListStudiosRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListStudiosRequest) ProtoMessage() {}

func (x *ListStudiosRequest) ProtoReflect() protoreflect.Message {
	mi := &file_studio_proto_msgTypes[19]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListStudiosRequest.ProtoReflect.Descriptor instead.
func (*ListStudiosRequest) Descriptor() ([]byte, []int) {
	return file_studio_proto_rawDescGZIP(), []int{19}
}

func (x *ListStudiosRequest) GetPageSize() int32 {
	if x != nil {
		return x.PageSize
	}
	return 0
}

func (x *ListStudiosRequest) GetPageNumber() int32 {
	if x != nil {
		return x.PageNumber
	}
	return 0
}

func (x *ListStudiosRequest) GetFilter() string {
	if x != nil {
		return x.Filter
	}
	return ""
}

func (x *ListStudiosRequest) GetOwnerId() string {
	if x != nil {
		return x.OwnerId
	}
	return ""
}

func (x *ListStudiosRequest) GetRequest() *BaseRequest {
	if x != nil {
		return x.Request
	}
	return nil
}

type ListStudiosResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Studios       []*Studio              `protobuf:"bytes,1,rep,name=studios,proto3" json:"studios,omitempty"`
	TotalCount    int32                  `protobuf:"varint,2,opt,name=total_count,json=totalCount,proto3" json:"total_count,omitempty"`
	PageNumber    int32                  `protobuf:"varint,3,opt,name=page_number,json=pageNumber,proto3" json:"page_number,omitempty"`
	TotalPages    int32                  `protobuf:"varint,4,opt,name=total_pages,json=totalPages,proto3" json:"total_pages,omitempty"`
	Response      *BaseResponse          `protobuf:"bytes,100,opt,name=response,proto3" json:"response,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ListStudiosResponse) Reset() {
	*x = ListStudiosResponse{}
	mi := &file_studio_proto_msgTypes[20]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ListStudiosResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListStudiosResponse) ProtoMessage() {}

func (x *ListStudiosResponse) ProtoReflect() protoreflect.Message {
	mi := &file_studio_proto_msgTypes[20]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListStudiosResponse.ProtoReflect.Descriptor instead.
func (*ListStudiosResponse) Descriptor() ([]byte, []int) {
	return file_studio_proto_rawDescGZIP(), []int{20}
}

func (x *ListStudiosResponse) GetStudios() []*Studio {
	if x != nil {
		return x.Studios
	}
	return nil
}

func (x *ListStudiosResponse) GetTotalCount() int32 {
	if x != nil {
		return x.TotalCount
	}
	return 0
}

func (x *ListStudiosResponse) GetPageNumber() int32 {
	if x != nil {
		return x.PageNumber
	}
	return 0
}

func (x *ListStudiosResponse) GetTotalPages() int32 {
	if x != nil {
		return x.TotalPages
	}
	return 0
}

func (x *ListStudiosResponse) GetResponse() *BaseResponse {
	if x != nil {
		return x.Response
	}
	return nil
}

type RegisterRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Username      string                 `protobuf:"bytes,1,opt,name=username,proto3" json:"username,omitempty"`
	Email         string                 `protobuf:"bytes,2,opt,name=email,proto3" json:"email,omitempty"`
	Password      string                 `protobuf:"bytes,3,opt,name=password,proto3" json:"password,omitempty"`
	StudioId      string                 `protobuf:"bytes,4,opt,name=studio_id,json=studioId,proto3" json:"studio_id,omitempty"`
	Role          string                 `protobuf:"bytes,5,opt,name=role,proto3" json:"role,omitempty"`
	CreatedAt     *timestamppb.Timestamp `protobuf:"bytes,6,opt,name=created_at,json=createdAt,proto3" json:"created_at,omitempty"`
	Request       *BaseRequest           `protobuf:"bytes,100,opt,name=request,proto3" json:"request,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *RegisterRequest) Reset() {
	*x = RegisterRequest{}
	mi := &file_studio_proto_msgTypes[21]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *RegisterRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*RegisterRequest) ProtoMessage() {}

func (x *RegisterRequest) ProtoReflect() protoreflect.Message {
	mi := &file_studio_proto_msgTypes[21]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use RegisterRequest.ProtoReflect.Descriptor instead.
func (*RegisterRequest) Descriptor() ([]byte, []int) {
	return file_studio_proto_rawDescGZIP(), []int{21}
}

func (x *RegisterRequest) GetUsername() string {
	if x != nil {
		return x.Username
	}
	return ""
}

func (x *RegisterRequest) GetEmail() string {
	if x != nil {
		return x.Email
	}
	return ""
}

func (x *RegisterRequest) GetPassword() string {
	if x != nil {
		return x.Password
	}
	return ""
}

func (x *RegisterRequest) GetStudioId() string {
	if x != nil {
		return x.StudioId
	}
	return ""
}

func (x *RegisterRequest) GetRole() string {
	if x != nil {
		return x.Role
	}
	return ""
}

func (x *RegisterRequest) GetCreatedAt() *timestamppb.Timestamp {
	if x != nil {
		return x.CreatedAt
	}
	return nil
}

func (x *RegisterRequest) GetRequest() *BaseRequest {
	if x != nil {
		return x.Request
	}
	return nil
}

type RegisterResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Message       string                 `protobuf:"bytes,1,opt,name=message,proto3" json:"message,omitempty"`
	Response      *BaseResponse          `protobuf:"bytes,100,opt,name=response,proto3" json:"response,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *RegisterResponse) Reset() {
	*x = RegisterResponse{}
	mi := &file_studio_proto_msgTypes[22]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *RegisterResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*RegisterResponse) ProtoMessage() {}

func (x *RegisterResponse) ProtoReflect() protoreflect.Message {
	mi := &file_studio_proto_msgTypes[22]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use RegisterResponse.ProtoReflect.Descriptor instead.
func (*RegisterResponse) Descriptor() ([]byte, []int) {
	return file_studio_proto_rawDescGZIP(), []int{22}
}

func (x *RegisterResponse) GetMessage() string {
	if x != nil {
		return x.Message
	}
	return ""
}

func (x *RegisterResponse) GetResponse() *BaseResponse {
	if x != nil {
		return x.Response
	}
	return nil
}

type LoginRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Username      string                 `protobuf:"bytes,1,opt,name=username,proto3" json:"username,omitempty"`
	Password      string                 `protobuf:"bytes,2,opt,name=password,proto3" json:"password,omitempty"`
	Email         string                 `protobuf:"bytes,3,opt,name=email,proto3" json:"email,omitempty"`
	Request       *BaseRequest           `protobuf:"bytes,100,opt,name=request,proto3" json:"request,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *LoginRequest) Reset() {
	*x = LoginRequest{}
	mi := &file_studio_proto_msgTypes[23]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *LoginRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*LoginRequest) ProtoMessage() {}

func (x *LoginRequest) ProtoReflect() protoreflect.Message {
	mi := &file_studio_proto_msgTypes[23]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use LoginRequest.ProtoReflect.Descriptor instead.
func (*LoginRequest) Descriptor() ([]byte, []int) {
	return file_studio_proto_rawDescGZIP(), []int{23}
}

func (x *LoginRequest) GetUsername() string {
	if x != nil {
		return x.Username
	}
	return ""
}

func (x *LoginRequest) GetPassword() string {
	if x != nil {
		return x.Password
	}
	return ""
}

func (x *LoginRequest) GetEmail() string {
	if x != nil {
		return x.Email
	}
	return ""
}

func (x *LoginRequest) GetRequest() *BaseRequest {
	if x != nil {
		return x.Request
	}
	return nil
}

type LoginResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	AccessToken   string                 `protobuf:"bytes,1,opt,name=access_token,json=accessToken,proto3" json:"access_token,omitempty"`
	RefreshToken  string                 `protobuf:"bytes,2,opt,name=refresh_token,json=refreshToken,proto3" json:"refresh_token,omitempty"`
	Message       string                 `protobuf:"bytes,3,opt,name=message,proto3" json:"message,omitempty"`
	Success       bool                   `protobuf:"varint,4,opt,name=success,proto3" json:"success,omitempty"`
	Response      *BaseResponse          `protobuf:"bytes,100,opt,name=response,proto3" json:"response,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *LoginResponse) Reset() {
	*x = LoginResponse{}
	mi := &file_studio_proto_msgTypes[24]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *LoginResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*LoginResponse) ProtoMessage() {}

func (x *LoginResponse) ProtoReflect() protoreflect.Message {
	mi := &file_studio_proto_msgTypes[24]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use LoginResponse.ProtoReflect.Descriptor instead.
func (*LoginResponse) Descriptor() ([]byte, []int) {
	return file_studio_proto_rawDescGZIP(), []int{24}
}

func (x *LoginResponse) GetAccessToken() string {
	if x != nil {
		return x.AccessToken
	}
	return ""
}

func (x *LoginResponse) GetRefreshToken() string {
	if x != nil {
		return x.RefreshToken
	}
	return ""
}

func (x *LoginResponse) GetMessage() string {
	if x != nil {
		return x.Message
	}
	return ""
}

func (x *LoginResponse) GetSuccess() bool {
	if x != nil {
		return x.Success
	}
	return false
}

func (x *LoginResponse) GetResponse() *BaseResponse {
	if x != nil {
		return x.Response
	}
	return nil
}

type LogoutRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	SessionId     string                 `protobuf:"bytes,1,opt,name=session_id,json=sessionId,proto3" json:"session_id,omitempty"`
	Request       *BaseRequest           `protobuf:"bytes,100,opt,name=request,proto3" json:"request,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *LogoutRequest) Reset() {
	*x = LogoutRequest{}
	mi := &file_studio_proto_msgTypes[25]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *LogoutRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*LogoutRequest) ProtoMessage() {}

func (x *LogoutRequest) ProtoReflect() protoreflect.Message {
	mi := &file_studio_proto_msgTypes[25]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use LogoutRequest.ProtoReflect.Descriptor instead.
func (*LogoutRequest) Descriptor() ([]byte, []int) {
	return file_studio_proto_rawDescGZIP(), []int{25}
}

func (x *LogoutRequest) GetSessionId() string {
	if x != nil {
		return x.SessionId
	}
	return ""
}

func (x *LogoutRequest) GetRequest() *BaseRequest {
	if x != nil {
		return x.Request
	}
	return nil
}

type LogoutResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Success       bool                   `protobuf:"varint,1,opt,name=success,proto3" json:"success,omitempty"`
	Message       string                 `protobuf:"bytes,2,opt,name=message,proto3" json:"message,omitempty"`
	Response      *BaseResponse          `protobuf:"bytes,100,opt,name=response,proto3" json:"response,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *LogoutResponse) Reset() {
	*x = LogoutResponse{}
	mi := &file_studio_proto_msgTypes[26]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *LogoutResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*LogoutResponse) ProtoMessage() {}

func (x *LogoutResponse) ProtoReflect() protoreflect.Message {
	mi := &file_studio_proto_msgTypes[26]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use LogoutResponse.ProtoReflect.Descriptor instead.
func (*LogoutResponse) Descriptor() ([]byte, []int) {
	return file_studio_proto_rawDescGZIP(), []int{26}
}

func (x *LogoutResponse) GetSuccess() bool {
	if x != nil {
		return x.Success
	}
	return false
}

func (x *LogoutResponse) GetMessage() string {
	if x != nil {
		return x.Message
	}
	return ""
}

func (x *LogoutResponse) GetResponse() *BaseResponse {
	if x != nil {
		return x.Response
	}
	return nil
}

type ValidateSessionRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	SessionId     string                 `protobuf:"bytes,1,opt,name=session_id,json=sessionId,proto3" json:"session_id,omitempty"`
	Request       *BaseRequest           `protobuf:"bytes,100,opt,name=request,proto3" json:"request,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ValidateSessionRequest) Reset() {
	*x = ValidateSessionRequest{}
	mi := &file_studio_proto_msgTypes[27]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ValidateSessionRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ValidateSessionRequest) ProtoMessage() {}

func (x *ValidateSessionRequest) ProtoReflect() protoreflect.Message {
	mi := &file_studio_proto_msgTypes[27]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ValidateSessionRequest.ProtoReflect.Descriptor instead.
func (*ValidateSessionRequest) Descriptor() ([]byte, []int) {
	return file_studio_proto_rawDescGZIP(), []int{27}
}

func (x *ValidateSessionRequest) GetSessionId() string {
	if x != nil {
		return x.SessionId
	}
	return ""
}

func (x *ValidateSessionRequest) GetRequest() *BaseRequest {
	if x != nil {
		return x.Request
	}
	return nil
}

type ValidateSessionResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Valid         bool                   `protobuf:"varint,1,opt,name=valid,proto3" json:"valid,omitempty"`
	UserId        string                 `protobuf:"bytes,2,opt,name=user_id,json=userId,proto3" json:"user_id,omitempty"`
	Username      string                 `protobuf:"bytes,3,opt,name=username,proto3" json:"username,omitempty"`
	Email         string                 `protobuf:"bytes,4,opt,name=email,proto3" json:"email,omitempty"`
	Permissions   []string               `protobuf:"bytes,5,rep,name=permissions,proto3" json:"permissions,omitempty"`
	Role          string                 `protobuf:"bytes,6,opt,name=role,proto3" json:"role,omitempty"`
	Response      *BaseResponse          `protobuf:"bytes,100,opt,name=response,proto3" json:"response,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ValidateSessionResponse) Reset() {
	*x = ValidateSessionResponse{}
	mi := &file_studio_proto_msgTypes[28]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ValidateSessionResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ValidateSessionResponse) ProtoMessage() {}

func (x *ValidateSessionResponse) ProtoReflect() protoreflect.Message {
	mi := &file_studio_proto_msgTypes[28]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ValidateSessionResponse.ProtoReflect.Descriptor instead.
func (*ValidateSessionResponse) Descriptor() ([]byte, []int) {
	return file_studio_proto_rawDescGZIP(), []int{28}
}

func (x *ValidateSessionResponse) GetValid() bool {
	if x != nil {
		return x.Valid
	}
	return false
}

func (x *ValidateSessionResponse) GetUserId() string {
	if x != nil {
		return x.UserId
	}
	return ""
}

func (x *ValidateSessionResponse) GetUsername() string {
	if x != nil {
		return x.Username
	}
	return ""
}

func (x *ValidateSessionResponse) GetEmail() string {
	if x != nil {
		return x.Email
	}
	return ""
}

func (x *ValidateSessionResponse) GetPermissions() []string {
	if x != nil {
		return x.Permissions
	}
	return nil
}

func (x *ValidateSessionResponse) GetRole() string {
	if x != nil {
		return x.Role
	}
	return ""
}

func (x *ValidateSessionResponse) GetResponse() *BaseResponse {
	if x != nil {
		return x.Response
	}
	return nil
}

type ChangeOwnPasswordRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	OldPassword   string                 `protobuf:"bytes,1,opt,name=old_password,json=oldPassword,proto3" json:"old_password,omitempty"`
	NewPassword   string                 `protobuf:"bytes,2,opt,name=new_password,json=newPassword,proto3" json:"new_password,omitempty"`
	Request       *BaseRequest           `protobuf:"bytes,100,opt,name=request,proto3" json:"request,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ChangeOwnPasswordRequest) Reset() {
	*x = ChangeOwnPasswordRequest{}
	mi := &file_studio_proto_msgTypes[29]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ChangeOwnPasswordRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ChangeOwnPasswordRequest) ProtoMessage() {}

func (x *ChangeOwnPasswordRequest) ProtoReflect() protoreflect.Message {
	mi := &file_studio_proto_msgTypes[29]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ChangeOwnPasswordRequest.ProtoReflect.Descriptor instead.
func (*ChangeOwnPasswordRequest) Descriptor() ([]byte, []int) {
	return file_studio_proto_rawDescGZIP(), []int{29}
}

func (x *ChangeOwnPasswordRequest) GetOldPassword() string {
	if x != nil {
		return x.OldPassword
	}
	return ""
}

func (x *ChangeOwnPasswordRequest) GetNewPassword() string {
	if x != nil {
		return x.NewPassword
	}
	return ""
}

func (x *ChangeOwnPasswordRequest) GetRequest() *BaseRequest {
	if x != nil {
		return x.Request
	}
	return nil
}

type ChangeOwnPasswordResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Message       string                 `protobuf:"bytes,1,opt,name=message,proto3" json:"message,omitempty"`
	Response      *BaseResponse          `protobuf:"bytes,100,opt,name=response,proto3" json:"response,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ChangeOwnPasswordResponse) Reset() {
	*x = ChangeOwnPasswordResponse{}
	mi := &file_studio_proto_msgTypes[30]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ChangeOwnPasswordResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ChangeOwnPasswordResponse) ProtoMessage() {}

func (x *ChangeOwnPasswordResponse) ProtoReflect() protoreflect.Message {
	mi := &file_studio_proto_msgTypes[30]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ChangeOwnPasswordResponse.ProtoReflect.Descriptor instead.
func (*ChangeOwnPasswordResponse) Descriptor() ([]byte, []int) {
	return file_studio_proto_rawDescGZIP(), []int{30}
}

func (x *ChangeOwnPasswordResponse) GetMessage() string {
	if x != nil {
		return x.Message
	}
	return ""
}

func (x *ChangeOwnPasswordResponse) GetResponse() *BaseResponse {
	if x != nil {
		return x.Response
	}
	return nil
}

type AdminResetUserPasswordRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	TargetUserId  string                 `protobuf:"bytes,1,opt,name=target_user_id,json=targetUserId,proto3" json:"target_user_id,omitempty"`
	NewPassword   string                 `protobuf:"bytes,2,opt,name=new_password,json=newPassword,proto3" json:"new_password,omitempty"`
	Request       *BaseRequest           `protobuf:"bytes,100,opt,name=request,proto3" json:"request,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *AdminResetUserPasswordRequest) Reset() {
	*x = AdminResetUserPasswordRequest{}
	mi := &file_studio_proto_msgTypes[31]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *AdminResetUserPasswordRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*AdminResetUserPasswordRequest) ProtoMessage() {}

func (x *AdminResetUserPasswordRequest) ProtoReflect() protoreflect.Message {
	mi := &file_studio_proto_msgTypes[31]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use AdminResetUserPasswordRequest.ProtoReflect.Descriptor instead.
func (*AdminResetUserPasswordRequest) Descriptor() ([]byte, []int) {
	return file_studio_proto_rawDescGZIP(), []int{31}
}

func (x *AdminResetUserPasswordRequest) GetTargetUserId() string {
	if x != nil {
		return x.TargetUserId
	}
	return ""
}

func (x *AdminResetUserPasswordRequest) GetNewPassword() string {
	if x != nil {
		return x.NewPassword
	}
	return ""
}

func (x *AdminResetUserPasswordRequest) GetRequest() *BaseRequest {
	if x != nil {
		return x.Request
	}
	return nil
}

type AdminResetUserPasswordResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Message       string                 `protobuf:"bytes,1,opt,name=message,proto3" json:"message,omitempty"`
	Response      *BaseResponse          `protobuf:"bytes,100,opt,name=response,proto3" json:"response,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *AdminResetUserPasswordResponse) Reset() {
	*x = AdminResetUserPasswordResponse{}
	mi := &file_studio_proto_msgTypes[32]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *AdminResetUserPasswordResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*AdminResetUserPasswordResponse) ProtoMessage() {}

func (x *AdminResetUserPasswordResponse) ProtoReflect() protoreflect.Message {
	mi := &file_studio_proto_msgTypes[32]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use AdminResetUserPasswordResponse.ProtoReflect.Descriptor instead.
func (*AdminResetUserPasswordResponse) Descriptor() ([]byte, []int) {
	return file_studio_proto_rawDescGZIP(), []int{32}
}

func (x *AdminResetUserPasswordResponse) GetMessage() string {
	if x != nil {
		return x.Message
	}
	return ""
}

func (x *AdminResetUserPasswordResponse) GetResponse() *BaseResponse {
	if x != nil {
		return x.Response
	}
	return nil
}

type ChangePasswordRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Username      string                 `protobuf:"bytes,1,opt,name=username,proto3" json:"username,omitempty"`
	OldPassword   string                 `protobuf:"bytes,2,opt,name=old_password,json=oldPassword,proto3" json:"old_password,omitempty"`
	NewPassword   string                 `protobuf:"bytes,3,opt,name=new_password,json=newPassword,proto3" json:"new_password,omitempty"`
	Request       *BaseRequest           `protobuf:"bytes,100,opt,name=request,proto3" json:"request,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ChangePasswordRequest) Reset() {
	*x = ChangePasswordRequest{}
	mi := &file_studio_proto_msgTypes[33]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ChangePasswordRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ChangePasswordRequest) ProtoMessage() {}

func (x *ChangePasswordRequest) ProtoReflect() protoreflect.Message {
	mi := &file_studio_proto_msgTypes[33]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ChangePasswordRequest.ProtoReflect.Descriptor instead.
func (*ChangePasswordRequest) Descriptor() ([]byte, []int) {
	return file_studio_proto_rawDescGZIP(), []int{33}
}

func (x *ChangePasswordRequest) GetUsername() string {
	if x != nil {
		return x.Username
	}
	return ""
}

func (x *ChangePasswordRequest) GetOldPassword() string {
	if x != nil {
		return x.OldPassword
	}
	return ""
}

func (x *ChangePasswordRequest) GetNewPassword() string {
	if x != nil {
		return x.NewPassword
	}
	return ""
}

func (x *ChangePasswordRequest) GetRequest() *BaseRequest {
	if x != nil {
		return x.Request
	}
	return nil
}

type ChangePasswordResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Message       string                 `protobuf:"bytes,1,opt,name=message,proto3" json:"message,omitempty"`
	Response      *BaseResponse          `protobuf:"bytes,100,opt,name=response,proto3" json:"response,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ChangePasswordResponse) Reset() {
	*x = ChangePasswordResponse{}
	mi := &file_studio_proto_msgTypes[34]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ChangePasswordResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ChangePasswordResponse) ProtoMessage() {}

func (x *ChangePasswordResponse) ProtoReflect() protoreflect.Message {
	mi := &file_studio_proto_msgTypes[34]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ChangePasswordResponse.ProtoReflect.Descriptor instead.
func (*ChangePasswordResponse) Descriptor() ([]byte, []int) {
	return file_studio_proto_rawDescGZIP(), []int{34}
}

func (x *ChangePasswordResponse) GetMessage() string {
	if x != nil {
		return x.Message
	}
	return ""
}

func (x *ChangePasswordResponse) GetResponse() *BaseResponse {
	if x != nil {
		return x.Response
	}
	return nil
}

type ChangeEmailRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Username      string                 `protobuf:"bytes,1,opt,name=username,proto3" json:"username,omitempty"`
	Password      string                 `protobuf:"bytes,2,opt,name=password,proto3" json:"password,omitempty"`
	NewEmail      string                 `protobuf:"bytes,3,opt,name=new_email,json=newEmail,proto3" json:"new_email,omitempty"`
	Request       *BaseRequest           `protobuf:"bytes,100,opt,name=request,proto3" json:"request,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ChangeEmailRequest) Reset() {
	*x = ChangeEmailRequest{}
	mi := &file_studio_proto_msgTypes[35]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ChangeEmailRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ChangeEmailRequest) ProtoMessage() {}

func (x *ChangeEmailRequest) ProtoReflect() protoreflect.Message {
	mi := &file_studio_proto_msgTypes[35]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ChangeEmailRequest.ProtoReflect.Descriptor instead.
func (*ChangeEmailRequest) Descriptor() ([]byte, []int) {
	return file_studio_proto_rawDescGZIP(), []int{35}
}

func (x *ChangeEmailRequest) GetUsername() string {
	if x != nil {
		return x.Username
	}
	return ""
}

func (x *ChangeEmailRequest) GetPassword() string {
	if x != nil {
		return x.Password
	}
	return ""
}

func (x *ChangeEmailRequest) GetNewEmail() string {
	if x != nil {
		return x.NewEmail
	}
	return ""
}

func (x *ChangeEmailRequest) GetRequest() *BaseRequest {
	if x != nil {
		return x.Request
	}
	return nil
}

type ChangeEmailResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Message       string                 `protobuf:"bytes,1,opt,name=message,proto3" json:"message,omitempty"`
	Response      *BaseResponse          `protobuf:"bytes,100,opt,name=response,proto3" json:"response,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ChangeEmailResponse) Reset() {
	*x = ChangeEmailResponse{}
	mi := &file_studio_proto_msgTypes[36]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ChangeEmailResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ChangeEmailResponse) ProtoMessage() {}

func (x *ChangeEmailResponse) ProtoReflect() protoreflect.Message {
	mi := &file_studio_proto_msgTypes[36]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ChangeEmailResponse.ProtoReflect.Descriptor instead.
func (*ChangeEmailResponse) Descriptor() ([]byte, []int) {
	return file_studio_proto_rawDescGZIP(), []int{36}
}

func (x *ChangeEmailResponse) GetMessage() string {
	if x != nil {
		return x.Message
	}
	return ""
}

func (x *ChangeEmailResponse) GetResponse() *BaseResponse {
	if x != nil {
		return x.Response
	}
	return nil
}

type RefreshTokenRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	RefreshToken  string                 `protobuf:"bytes,1,opt,name=refresh_token,json=refreshToken,proto3" json:"refresh_token,omitempty"`
	Request       *BaseRequest           `protobuf:"bytes,100,opt,name=request,proto3" json:"request,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *RefreshTokenRequest) Reset() {
	*x = RefreshTokenRequest{}
	mi := &file_studio_proto_msgTypes[37]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *RefreshTokenRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*RefreshTokenRequest) ProtoMessage() {}

func (x *RefreshTokenRequest) ProtoReflect() protoreflect.Message {
	mi := &file_studio_proto_msgTypes[37]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use RefreshTokenRequest.ProtoReflect.Descriptor instead.
func (*RefreshTokenRequest) Descriptor() ([]byte, []int) {
	return file_studio_proto_rawDescGZIP(), []int{37}
}

func (x *RefreshTokenRequest) GetRefreshToken() string {
	if x != nil {
		return x.RefreshToken
	}
	return ""
}

func (x *RefreshTokenRequest) GetRequest() *BaseRequest {
	if x != nil {
		return x.Request
	}
	return nil
}

type TokenResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	AccessToken   string                 `protobuf:"bytes,1,opt,name=access_token,json=accessToken,proto3" json:"access_token,omitempty"`
	RefreshToken  string                 `protobuf:"bytes,2,opt,name=refresh_token,json=refreshToken,proto3" json:"refresh_token,omitempty"`
	Response      *BaseResponse          `protobuf:"bytes,100,opt,name=response,proto3" json:"response,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *TokenResponse) Reset() {
	*x = TokenResponse{}
	mi := &file_studio_proto_msgTypes[38]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *TokenResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*TokenResponse) ProtoMessage() {}

func (x *TokenResponse) ProtoReflect() protoreflect.Message {
	mi := &file_studio_proto_msgTypes[38]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use TokenResponse.ProtoReflect.Descriptor instead.
func (*TokenResponse) Descriptor() ([]byte, []int) {
	return file_studio_proto_rawDescGZIP(), []int{38}
}

func (x *TokenResponse) GetAccessToken() string {
	if x != nil {
		return x.AccessToken
	}
	return ""
}

func (x *TokenResponse) GetRefreshToken() string {
	if x != nil {
		return x.RefreshToken
	}
	return ""
}

func (x *TokenResponse) GetResponse() *BaseResponse {
	if x != nil {
		return x.Response
	}
	return nil
}

type BaseRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Downstream    string                 `protobuf:"bytes,998,opt,name=downstream,proto3" json:"downstream,omitempty"`
	RequestId     string                 `protobuf:"bytes,999,opt,name=request_id,json=requestId,proto3" json:"request_id,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *BaseRequest) Reset() {
	*x = BaseRequest{}
	mi := &file_studio_proto_msgTypes[39]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *BaseRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*BaseRequest) ProtoMessage() {}

func (x *BaseRequest) ProtoReflect() protoreflect.Message {
	mi := &file_studio_proto_msgTypes[39]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use BaseRequest.ProtoReflect.Descriptor instead.
func (*BaseRequest) Descriptor() ([]byte, []int) {
	return file_studio_proto_rawDescGZIP(), []int{39}
}

func (x *BaseRequest) GetDownstream() string {
	if x != nil {
		return x.Downstream
	}
	return ""
}

func (x *BaseRequest) GetRequestId() string {
	if x != nil {
		return x.RequestId
	}
	return ""
}

type BaseResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Upstream      string                 `protobuf:"bytes,998,opt,name=upstream,proto3" json:"upstream,omitempty"`
	RequestId     string                 `protobuf:"bytes,999,opt,name=request_id,json=requestId,proto3" json:"request_id,omitempty"`
	Status        string                 `protobuf:"bytes,1000,opt,name=status,proto3" json:"status,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *BaseResponse) Reset() {
	*x = BaseResponse{}
	mi := &file_studio_proto_msgTypes[40]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *BaseResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*BaseResponse) ProtoMessage() {}

func (x *BaseResponse) ProtoReflect() protoreflect.Message {
	mi := &file_studio_proto_msgTypes[40]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use BaseResponse.ProtoReflect.Descriptor instead.
func (*BaseResponse) Descriptor() ([]byte, []int) {
	return file_studio_proto_rawDescGZIP(), []int{40}
}

func (x *BaseResponse) GetUpstream() string {
	if x != nil {
		return x.Upstream
	}
	return ""
}

func (x *BaseResponse) GetRequestId() string {
	if x != nil {
		return x.RequestId
	}
	return ""
}

func (x *BaseResponse) GetStatus() string {
	if x != nil {
		return x.Status
	}
	return ""
}

var File_studio_proto protoreflect.FileDescriptor

const file_studio_proto_rawDesc = "" +
	"\n" +
	"\fstudio.proto\x12\finkMe.studio\x1a google/protobuf/field_mask.proto\x1a\x1fgoogle/protobuf/timestamp.proto\"x\n" +
	"\x13CreateStudioRequest\x12,\n" +
	"\x06studio\x18\x01 \x01(\v2\x14.inkMe.studio.StudioR\x06studio\x123\n" +
	"\arequest\x18d \x01(\v2\x19.inkMe.studio.BaseRequestR\arequest\"\x85\x01\n" +
	"\x14CreateStudioResponse\x12\x1b\n" +
	"\tstudio_id\x18\x01 \x01(\tR\bstudioId\x12\x18\n" +
	"\amessage\x18\x02 \x01(\tR\amessage\x126\n" +
	"\bresponse\x18d \x01(\v2\x1a.inkMe.studio.BaseResponseR\bresponse\"\xcc\x01\n" +
	"\x13UpdateStudioRequest\x12\x1b\n" +
	"\tstudio_id\x18\x01 \x01(\tR\bstudioId\x12,\n" +
	"\x06studio\x18\x02 \x01(\v2\x14.inkMe.studio.StudioR\x06studio\x12;\n" +
	"\vupdate_mask\x18\x03 \x01(\v2\x1a.google.protobuf.FieldMaskR\n" +
	"updateMask\x12-\n" +
	"\x04base\x18d \x01(\v2\x19.inkMe.studio.BaseRequestR\x04base\"\xb0\x01\n" +
	"\x14UpdateStudioResponse\x12\x18\n" +
	"\asuccess\x18\x01 \x01(\bR\asuccess\x12\x18\n" +
	"\amessage\x18\x02 \x01(\tR\amessage\x12,\n" +
	"\x06studio\x18\x03 \x01(\v2\x14.inkMe.studio.StudioR\x06studio\x126\n" +
	"\bresponse\x18d \x01(\v2\x1a.inkMe.studio.BaseResponseR\bresponse\"\xb8\x01\n" +
	"\x15AddStaffMemberRequest\x12\x1b\n" +
	"\tstudio_id\x18\x01 \x01(\tR\bstudioId\x12\x17\n" +
	"\auser_id\x18\x02 \x01(\tR\x06userId\x12\x12\n" +
	"\x04role\x18\x03 \x01(\tR\x04role\x12 \n" +
	"\vpermissions\x18\x04 \x03(\tR\vpermissions\x123\n" +
	"\arequest\x18d \x01(\v2\x19.inkMe.studio.BaseRequestR\arequest\"\x85\x01\n" +
	"\x16AddStaffMemberResponse\x12\x19\n" +
	"\bstaff_id\x18\x01 \x01(\tR\astaffId\x12\x18\n" +
	"\amessage\x18\x02 \x01(\tR\amessage\x126\n" +
	"\bresponse\x18d \x01(\v2\x1a.inkMe.studio.BaseResponseR\bresponse\"~\n" +
	"\x18UpdateStaffMemberRequest\x12\x19\n" +
	"\bstaff_id\x18\x01 \x01(\tR\astaffId\x12\x12\n" +
	"\x04role\x18\x02 \x01(\tR\x04role\x123\n" +
	"\arequest\x18d \x01(\v2\x19.inkMe.studio.BaseRequestR\arequest\"m\n" +
	"\x19UpdateStaffMemberResponse\x12\x18\n" +
	"\amessage\x18\x01 \x01(\tR\amessage\x126\n" +
	"\bresponse\x18d \x01(\v2\x1a.inkMe.studio.BaseResponseR\bresponse\"j\n" +
	"\x18RemoveStaffMemberRequest\x12\x19\n" +
	"\bstaff_id\x18\x01 \x01(\tR\astaffId\x123\n" +
	"\arequest\x18d \x01(\v2\x19.inkMe.studio.BaseRequestR\arequest\"m\n" +
	"\x19RemoveStaffMemberResponse\x12\x18\n" +
	"\amessage\x18\x01 \x01(\tR\amessage\x126\n" +
	"\bresponse\x18d \x01(\v2\x1a.inkMe.studio.BaseResponseR\bresponse\"k\n" +
	"\x17ListStaffMembersRequest\x12\x1b\n" +
	"\tstudio_id\x18\x01 \x01(\tR\bstudioId\x123\n" +
	"\arequest\x18d \x01(\v2\x19.inkMe.studio.BaseRequestR\arequest\"\x92\x01\n" +
	"\x18ListStaffMembersResponse\x12>\n" +
	"\rstaff_members\x18\x01 \x03(\v2\x19.inkMe.studio.StaffMemberR\fstaffMembers\x126\n" +
	"\bresponse\x18d \x01(\v2\x1a.inkMe.studio.BaseResponseR\bresponse\"\x8e\x01\n" +
	"\x1aSetStaffPermissionsRequest\x12\x19\n" +
	"\bstaff_id\x18\x01 \x01(\tR\astaffId\x12 \n" +
	"\vpermissions\x18\x02 \x03(\tR\vpermissions\x123\n" +
	"\arequest\x18d \x01(\v2\x19.inkMe.studio.BaseRequestR\arequest\"o\n" +
	"\x1bSetStaffPermissionsResponse\x12\x18\n" +
	"\amessage\x18\x01 \x01(\tR\amessage\x126\n" +
	"\bresponse\x18d \x01(\v2\x1a.inkMe.studio.BaseResponseR\bresponse\"l\n" +
	"\x1aGetStaffPermissionsRequest\x12\x19\n" +
	"\bstaff_id\x18\x01 \x01(\tR\astaffId\x123\n" +
	"\arequest\x18d \x01(\v2\x19.inkMe.studio.BaseRequestR\arequest\"w\n" +
	"\x1bGetStaffPermissionsResponse\x12 \n" +
	"\vpermissions\x18\x01 \x03(\tR\vpermissions\x126\n" +
	"\bresponse\x18d \x01(\v2\x1a.inkMe.studio.BaseResponseR\bresponse\"\xbe\x02\n" +
	"\x06Studio\x12\x1b\n" +
	"\tstudio_id\x18\x01 \x01(\tR\bstudioId\x12\x12\n" +
	"\x04name\x18\x02 \x01(\tR\x04name\x12\x18\n" +
	"\aaddress\x18\x03 \x01(\tR\aaddress\x12\x14\n" +
	"\x05phone\x18\x04 \x01(\tR\x05phone\x12\x14\n" +
	"\x05email\x18\x05 \x01(\tR\x05email\x12\x18\n" +
	"\awebsite\x18\x06 \x01(\tR\awebsite\x12-\n" +
	"\x05owner\x18\a \x01(\v2\x17.inkMe.studio.OwnerInfoR\x05owner\x129\n" +
	"\n" +
	"created_at\x18\b \x01(\v2\x1a.google.protobuf.TimestampR\tcreatedAt\x129\n" +
	"\n" +
	"updated_at\x18\t \x01(\v2\x1a.google.protobuf.TimestampR\tupdatedAt\"\x8a\x02\n" +
	"\vStaffMember\x12\x19\n" +
	"\bstaff_id\x18\x01 \x01(\tR\astaffId\x12\x1b\n" +
	"\tstudio_id\x18\x02 \x01(\tR\bstudioId\x12\x17\n" +
	"\auser_id\x18\x03 \x01(\tR\x06userId\x12\x12\n" +
	"\x04role\x18\x04 \x01(\tR\x04role\x12 \n" +
	"\vpermissions\x18\x05 \x03(\tR\vpermissions\x129\n" +
	"\n" +
	"created_at\x18\x06 \x01(\v2\x1a.google.protobuf.TimestampR\tcreatedAt\x129\n" +
	"\n" +
	"updated_at\x18\a \x01(\v2\x1a.google.protobuf.TimestampR\tupdatedAt\"\xb8\x01\n" +
	"\tOwnerInfo\x12\x14\n" +
	"\x05email\x18\x01 \x01(\tR\x05email\x12\x1a\n" +
	"\bpassword\x18\x02 \x01(\tR\bpassword\x12!\n" +
	"\fdisplay_name\x18\x03 \x01(\tR\vdisplayName\x12\x1a\n" +
	"\busername\x18\x04 \x01(\tR\busername\x12\x1d\n" +
	"\n" +
	"first_name\x18\x05 \x01(\tR\tfirstName\x12\x1b\n" +
	"\tlast_name\x18\x06 \x01(\tR\blastName\"\xba\x01\n" +
	"\x12ListStudiosRequest\x12\x1b\n" +
	"\tpage_size\x18\x01 \x01(\x05R\bpageSize\x12\x1f\n" +
	"\vpage_number\x18\x02 \x01(\x05R\n" +
	"pageNumber\x12\x16\n" +
	"\x06filter\x18\x03 \x01(\tR\x06filter\x12\x19\n" +
	"\bowner_id\x18\x04 \x01(\tR\aownerId\x123\n" +
	"\arequest\x18d \x01(\v2\x19.inkMe.studio.BaseRequestR\arequest\"\xe0\x01\n" +
	"\x13ListStudiosResponse\x12.\n" +
	"\astudios\x18\x01 \x03(\v2\x14.inkMe.studio.StudioR\astudios\x12\x1f\n" +
	"\vtotal_count\x18\x02 \x01(\x05R\n" +
	"totalCount\x12\x1f\n" +
	"\vpage_number\x18\x03 \x01(\x05R\n" +
	"pageNumber\x12\x1f\n" +
	"\vtotal_pages\x18\x04 \x01(\x05R\n" +
	"totalPages\x126\n" +
	"\bresponse\x18d \x01(\v2\x1a.inkMe.studio.BaseResponseR\bresponse\"\x80\x02\n" +
	"\x0fRegisterRequest\x12\x1a\n" +
	"\busername\x18\x01 \x01(\tR\busername\x12\x14\n" +
	"\x05email\x18\x02 \x01(\tR\x05email\x12\x1a\n" +
	"\bpassword\x18\x03 \x01(\tR\bpassword\x12\x1b\n" +
	"\tstudio_id\x18\x04 \x01(\tR\bstudioId\x12\x12\n" +
	"\x04role\x18\x05 \x01(\tR\x04role\x129\n" +
	"\n" +
	"created_at\x18\x06 \x01(\v2\x1a.google.protobuf.TimestampR\tcreatedAt\x123\n" +
	"\arequest\x18d \x01(\v2\x19.inkMe.studio.BaseRequestR\arequest\"d\n" +
	"\x10RegisterResponse\x12\x18\n" +
	"\amessage\x18\x01 \x01(\tR\amessage\x126\n" +
	"\bresponse\x18d \x01(\v2\x1a.inkMe.studio.BaseResponseR\bresponse\"\x91\x01\n" +
	"\fLoginRequest\x12\x1a\n" +
	"\busername\x18\x01 \x01(\tR\busername\x12\x1a\n" +
	"\bpassword\x18\x02 \x01(\tR\bpassword\x12\x14\n" +
	"\x05email\x18\x03 \x01(\tR\x05email\x123\n" +
	"\arequest\x18d \x01(\v2\x19.inkMe.studio.BaseRequestR\arequest\"\xc3\x01\n" +
	"\rLoginResponse\x12!\n" +
	"\faccess_token\x18\x01 \x01(\tR\vaccessToken\x12#\n" +
	"\rrefresh_token\x18\x02 \x01(\tR\frefreshToken\x12\x18\n" +
	"\amessage\x18\x03 \x01(\tR\amessage\x12\x18\n" +
	"\asuccess\x18\x04 \x01(\bR\asuccess\x126\n" +
	"\bresponse\x18d \x01(\v2\x1a.inkMe.studio.BaseResponseR\bresponse\"c\n" +
	"\rLogoutRequest\x12\x1d\n" +
	"\n" +
	"session_id\x18\x01 \x01(\tR\tsessionId\x123\n" +
	"\arequest\x18d \x01(\v2\x19.inkMe.studio.BaseRequestR\arequest\"|\n" +
	"\x0eLogoutResponse\x12\x18\n" +
	"\asuccess\x18\x01 \x01(\bR\asuccess\x12\x18\n" +
	"\amessage\x18\x02 \x01(\tR\amessage\x126\n" +
	"\bresponse\x18d \x01(\v2\x1a.inkMe.studio.BaseResponseR\bresponse\"l\n" +
	"\x16ValidateSessionRequest\x12\x1d\n" +
	"\n" +
	"session_id\x18\x01 \x01(\tR\tsessionId\x123\n" +
	"\arequest\x18d \x01(\v2\x19.inkMe.studio.BaseRequestR\arequest\"\xe8\x01\n" +
	"\x17ValidateSessionResponse\x12\x14\n" +
	"\x05valid\x18\x01 \x01(\bR\x05valid\x12\x17\n" +
	"\auser_id\x18\x02 \x01(\tR\x06userId\x12\x1a\n" +
	"\busername\x18\x03 \x01(\tR\busername\x12\x14\n" +
	"\x05email\x18\x04 \x01(\tR\x05email\x12 \n" +
	"\vpermissions\x18\x05 \x03(\tR\vpermissions\x12\x12\n" +
	"\x04role\x18\x06 \x01(\tR\x04role\x126\n" +
	"\bresponse\x18d \x01(\v2\x1a.inkMe.studio.BaseResponseR\bresponse\"\x95\x01\n" +
	"\x18ChangeOwnPasswordRequest\x12!\n" +
	"\fold_password\x18\x01 \x01(\tR\voldPassword\x12!\n" +
	"\fnew_password\x18\x02 \x01(\tR\vnewPassword\x123\n" +
	"\arequest\x18d \x01(\v2\x19.inkMe.studio.BaseRequestR\arequest\"m\n" +
	"\x19ChangeOwnPasswordResponse\x12\x18\n" +
	"\amessage\x18\x01 \x01(\tR\amessage\x126\n" +
	"\bresponse\x18d \x01(\v2\x1a.inkMe.studio.BaseResponseR\bresponse\"\x9d\x01\n" +
	"\x1dAdminResetUserPasswordRequest\x12$\n" +
	"\x0etarget_user_id\x18\x01 \x01(\tR\ftargetUserId\x12!\n" +
	"\fnew_password\x18\x02 \x01(\tR\vnewPassword\x123\n" +
	"\arequest\x18d \x01(\v2\x19.inkMe.studio.BaseRequestR\arequest\"r\n" +
	"\x1eAdminResetUserPasswordResponse\x12\x18\n" +
	"\amessage\x18\x01 \x01(\tR\amessage\x126\n" +
	"\bresponse\x18d \x01(\v2\x1a.inkMe.studio.BaseResponseR\bresponse\"\xae\x01\n" +
	"\x15ChangePasswordRequest\x12\x1a\n" +
	"\busername\x18\x01 \x01(\tR\busername\x12!\n" +
	"\fold_password\x18\x02 \x01(\tR\voldPassword\x12!\n" +
	"\fnew_password\x18\x03 \x01(\tR\vnewPassword\x123\n" +
	"\arequest\x18d \x01(\v2\x19.inkMe.studio.BaseRequestR\arequest\"j\n" +
	"\x16ChangePasswordResponse\x12\x18\n" +
	"\amessage\x18\x01 \x01(\tR\amessage\x126\n" +
	"\bresponse\x18d \x01(\v2\x1a.inkMe.studio.BaseResponseR\bresponse\"\x9e\x01\n" +
	"\x12ChangeEmailRequest\x12\x1a\n" +
	"\busername\x18\x01 \x01(\tR\busername\x12\x1a\n" +
	"\bpassword\x18\x02 \x01(\tR\bpassword\x12\x1b\n" +
	"\tnew_email\x18\x03 \x01(\tR\bnewEmail\x123\n" +
	"\arequest\x18d \x01(\v2\x19.inkMe.studio.BaseRequestR\arequest\"g\n" +
	"\x13ChangeEmailResponse\x12\x18\n" +
	"\amessage\x18\x01 \x01(\tR\amessage\x126\n" +
	"\bresponse\x18d \x01(\v2\x1a.inkMe.studio.BaseResponseR\bresponse\"o\n" +
	"\x13RefreshTokenRequest\x12#\n" +
	"\rrefresh_token\x18\x01 \x01(\tR\frefreshToken\x123\n" +
	"\arequest\x18d \x01(\v2\x19.inkMe.studio.BaseRequestR\arequest\"\x8f\x01\n" +
	"\rTokenResponse\x12!\n" +
	"\faccess_token\x18\x01 \x01(\tR\vaccessToken\x12#\n" +
	"\rrefresh_token\x18\x02 \x01(\tR\frefreshToken\x126\n" +
	"\bresponse\x18d \x01(\v2\x1a.inkMe.studio.BaseResponseR\bresponse\"N\n" +
	"\vBaseRequest\x12\x1f\n" +
	"\n" +
	"downstream\x18\xe6\a \x01(\tR\n" +
	"downstream\x12\x1e\n" +
	"\n" +
	"request_id\x18\xe7\a \x01(\tR\trequestId\"d\n" +
	"\fBaseResponse\x12\x1b\n" +
	"\bupstream\x18\xe6\a \x01(\tR\bupstream\x12\x1e\n" +
	"\n" +
	"request_id\x18\xe7\a \x01(\tR\trequestId\x12\x17\n" +
	"\x06status\x18\xe8\a \x01(\tR\x06status2\xf5\x06\n" +
	"\rStudioService\x12U\n" +
	"\fCreateStudio\x12!.inkMe.studio.CreateStudioRequest\x1a\".inkMe.studio.CreateStudioResponse\x12U\n" +
	"\fUpdateStudio\x12!.inkMe.studio.UpdateStudioRequest\x1a\".inkMe.studio.UpdateStudioResponse\x12R\n" +
	"\vListStudios\x12 .inkMe.studio.ListStudiosRequest\x1a!.inkMe.studio.ListStudiosResponse\x12[\n" +
	"\x0eAddStaffMember\x12#.inkMe.studio.AddStaffMemberRequest\x1a$.inkMe.studio.AddStaffMemberResponse\x12d\n" +
	"\x11UpdateStaffMember\x12&.inkMe.studio.UpdateStaffMemberRequest\x1a'.inkMe.studio.UpdateStaffMemberResponse\x12d\n" +
	"\x11RemoveStaffMember\x12&.inkMe.studio.RemoveStaffMemberRequest\x1a'.inkMe.studio.RemoveStaffMemberResponse\x12a\n" +
	"\x10ListStaffMembers\x12%.inkMe.studio.ListStaffMembersRequest\x1a&.inkMe.studio.ListStaffMembersResponse\x12j\n" +
	"\x13SetStaffPermissions\x12(.inkMe.studio.SetStaffPermissionsRequest\x1a).inkMe.studio.SetStaffPermissionsResponse\x12j\n" +
	"\x13GetStaffPermissions\x12(.inkMe.studio.GetStaffPermissionsRequest\x1a).inkMe.studio.GetStaffPermissionsResponse2\x9b\x06\n" +
	"\vAuthService\x12I\n" +
	"\bRegister\x12\x1d.inkMe.studio.RegisterRequest\x1a\x1e.inkMe.studio.RegisterResponse\x12@\n" +
	"\x05Login\x12\x1a.inkMe.studio.LoginRequest\x1a\x1b.inkMe.studio.LoginResponse\x12C\n" +
	"\x06Logout\x12\x1b.inkMe.studio.LogoutRequest\x1a\x1c.inkMe.studio.LogoutResponse\x12[\n" +
	"\x0eChangePassword\x12#.inkMe.studio.ChangePasswordRequest\x1a$.inkMe.studio.ChangePasswordResponse\x12R\n" +
	"\vChangeEmail\x12 .inkMe.studio.ChangeEmailRequest\x1a!.inkMe.studio.ChangeEmailResponse\x12N\n" +
	"\fRefreshToken\x12!.inkMe.studio.RefreshTokenRequest\x1a\x1b.inkMe.studio.TokenResponse\x12^\n" +
	"\x0fValidateSession\x12$.inkMe.studio.ValidateSessionRequest\x1a%.inkMe.studio.ValidateSessionResponse\x12d\n" +
	"\x11ChangeOwnPassword\x12&.inkMe.studio.ChangeOwnPasswordRequest\x1a'.inkMe.studio.ChangeOwnPasswordResponse\x12s\n" +
	"\x16AdminResetUserPassword\x12+.inkMe.studio.AdminResetUserPasswordRequest\x1a,.inkMe.studio.AdminResetUserPasswordResponseBGZEgithub.com/FACorreiaa/ink-app-backend-protos/modules/studio/generatedb\x06proto3"

var (
	file_studio_proto_rawDescOnce sync.Once
	file_studio_proto_rawDescData []byte
)

func file_studio_proto_rawDescGZIP() []byte {
	file_studio_proto_rawDescOnce.Do(func() {
		file_studio_proto_rawDescData = protoimpl.X.CompressGZIP(unsafe.Slice(unsafe.StringData(file_studio_proto_rawDesc), len(file_studio_proto_rawDesc)))
	})
	return file_studio_proto_rawDescData
}

var file_studio_proto_msgTypes = make([]protoimpl.MessageInfo, 41)
var file_studio_proto_goTypes = []any{
	(*CreateStudioRequest)(nil),            // 0: inkMe.studio.CreateStudioRequest
	(*CreateStudioResponse)(nil),           // 1: inkMe.studio.CreateStudioResponse
	(*UpdateStudioRequest)(nil),            // 2: inkMe.studio.UpdateStudioRequest
	(*UpdateStudioResponse)(nil),           // 3: inkMe.studio.UpdateStudioResponse
	(*AddStaffMemberRequest)(nil),          // 4: inkMe.studio.AddStaffMemberRequest
	(*AddStaffMemberResponse)(nil),         // 5: inkMe.studio.AddStaffMemberResponse
	(*UpdateStaffMemberRequest)(nil),       // 6: inkMe.studio.UpdateStaffMemberRequest
	(*UpdateStaffMemberResponse)(nil),      // 7: inkMe.studio.UpdateStaffMemberResponse
	(*RemoveStaffMemberRequest)(nil),       // 8: inkMe.studio.RemoveStaffMemberRequest
	(*RemoveStaffMemberResponse)(nil),      // 9: inkMe.studio.RemoveStaffMemberResponse
	(*ListStaffMembersRequest)(nil),        // 10: inkMe.studio.ListStaffMembersRequest
	(*ListStaffMembersResponse)(nil),       // 11: inkMe.studio.ListStaffMembersResponse
	(*SetStaffPermissionsRequest)(nil),     // 12: inkMe.studio.SetStaffPermissionsRequest
	(*SetStaffPermissionsResponse)(nil),    // 13: inkMe.studio.SetStaffPermissionsResponse
	(*GetStaffPermissionsRequest)(nil),     // 14: inkMe.studio.GetStaffPermissionsRequest
	(*GetStaffPermissionsResponse)(nil),    // 15: inkMe.studio.GetStaffPermissionsResponse
	(*Studio)(nil),                         // 16: inkMe.studio.Studio
	(*StaffMember)(nil),                    // 17: inkMe.studio.StaffMember
	(*OwnerInfo)(nil),                      // 18: inkMe.studio.OwnerInfo
	(*ListStudiosRequest)(nil),             // 19: inkMe.studio.ListStudiosRequest
	(*ListStudiosResponse)(nil),            // 20: inkMe.studio.ListStudiosResponse
	(*RegisterRequest)(nil),                // 21: inkMe.studio.RegisterRequest
	(*RegisterResponse)(nil),               // 22: inkMe.studio.RegisterResponse
	(*LoginRequest)(nil),                   // 23: inkMe.studio.LoginRequest
	(*LoginResponse)(nil),                  // 24: inkMe.studio.LoginResponse
	(*LogoutRequest)(nil),                  // 25: inkMe.studio.LogoutRequest
	(*LogoutResponse)(nil),                 // 26: inkMe.studio.LogoutResponse
	(*ValidateSessionRequest)(nil),         // 27: inkMe.studio.ValidateSessionRequest
	(*ValidateSessionResponse)(nil),        // 28: inkMe.studio.ValidateSessionResponse
	(*ChangeOwnPasswordRequest)(nil),       // 29: inkMe.studio.ChangeOwnPasswordRequest
	(*ChangeOwnPasswordResponse)(nil),      // 30: inkMe.studio.ChangeOwnPasswordResponse
	(*AdminResetUserPasswordRequest)(nil),  // 31: inkMe.studio.AdminResetUserPasswordRequest
	(*AdminResetUserPasswordResponse)(nil), // 32: inkMe.studio.AdminResetUserPasswordResponse
	(*ChangePasswordRequest)(nil),          // 33: inkMe.studio.ChangePasswordRequest
	(*ChangePasswordResponse)(nil),         // 34: inkMe.studio.ChangePasswordResponse
	(*ChangeEmailRequest)(nil),             // 35: inkMe.studio.ChangeEmailRequest
	(*ChangeEmailResponse)(nil),            // 36: inkMe.studio.ChangeEmailResponse
	(*RefreshTokenRequest)(nil),            // 37: inkMe.studio.RefreshTokenRequest
	(*TokenResponse)(nil),                  // 38: inkMe.studio.TokenResponse
	(*BaseRequest)(nil),                    // 39: inkMe.studio.BaseRequest
	(*BaseResponse)(nil),                   // 40: inkMe.studio.BaseResponse
	(*fieldmaskpb.FieldMask)(nil),          // 41: google.protobuf.FieldMask
	(*timestamppb.Timestamp)(nil),          // 42: google.protobuf.Timestamp
}
var file_studio_proto_depIdxs = []int32{
	16, // 0: inkMe.studio.CreateStudioRequest.studio:type_name -> inkMe.studio.Studio
	39, // 1: inkMe.studio.CreateStudioRequest.request:type_name -> inkMe.studio.BaseRequest
	40, // 2: inkMe.studio.CreateStudioResponse.response:type_name -> inkMe.studio.BaseResponse
	16, // 3: inkMe.studio.UpdateStudioRequest.studio:type_name -> inkMe.studio.Studio
	41, // 4: inkMe.studio.UpdateStudioRequest.update_mask:type_name -> google.protobuf.FieldMask
	39, // 5: inkMe.studio.UpdateStudioRequest.base:type_name -> inkMe.studio.BaseRequest
	16, // 6: inkMe.studio.UpdateStudioResponse.studio:type_name -> inkMe.studio.Studio
	40, // 7: inkMe.studio.UpdateStudioResponse.response:type_name -> inkMe.studio.BaseResponse
	39, // 8: inkMe.studio.AddStaffMemberRequest.request:type_name -> inkMe.studio.BaseRequest
	40, // 9: inkMe.studio.AddStaffMemberResponse.response:type_name -> inkMe.studio.BaseResponse
	39, // 10: inkMe.studio.UpdateStaffMemberRequest.request:type_name -> inkMe.studio.BaseRequest
	40, // 11: inkMe.studio.UpdateStaffMemberResponse.response:type_name -> inkMe.studio.BaseResponse
	39, // 12: inkMe.studio.RemoveStaffMemberRequest.request:type_name -> inkMe.studio.BaseRequest
	40, // 13: inkMe.studio.RemoveStaffMemberResponse.response:type_name -> inkMe.studio.BaseResponse
	39, // 14: inkMe.studio.ListStaffMembersRequest.request:type_name -> inkMe.studio.BaseRequest
	17, // 15: inkMe.studio.ListStaffMembersResponse.staff_members:type_name -> inkMe.studio.StaffMember
	40, // 16: inkMe.studio.ListStaffMembersResponse.response:type_name -> inkMe.studio.BaseResponse
	39, // 17: inkMe.studio.SetStaffPermissionsRequest.request:type_name -> inkMe.studio.BaseRequest
	40, // 18: inkMe.studio.SetStaffPermissionsResponse.response:type_name -> inkMe.studio.BaseResponse
	39, // 19: inkMe.studio.GetStaffPermissionsRequest.request:type_name -> inkMe.studio.BaseRequest
	40, // 20: inkMe.studio.GetStaffPermissionsResponse.response:type_name -> inkMe.studio.BaseResponse
	18, // 21: inkMe.studio.Studio.owner:type_name -> inkMe.studio.OwnerInfo
	42, // 22: inkMe.studio.Studio.created_at:type_name -> google.protobuf.Timestamp
	42, // 23: inkMe.studio.Studio.updated_at:type_name -> google.protobuf.Timestamp
	42, // 24: inkMe.studio.StaffMember.created_at:type_name -> google.protobuf.Timestamp
	42, // 25: inkMe.studio.StaffMember.updated_at:type_name -> google.protobuf.Timestamp
	39, // 26: inkMe.studio.ListStudiosRequest.request:type_name -> inkMe.studio.BaseRequest
	16, // 27: inkMe.studio.ListStudiosResponse.studios:type_name -> inkMe.studio.Studio
	40, // 28: inkMe.studio.ListStudiosResponse.response:type_name -> inkMe.studio.BaseResponse
	42, // 29: inkMe.studio.RegisterRequest.created_at:type_name -> google.protobuf.Timestamp
	39, // 30: inkMe.studio.RegisterRequest.request:type_name -> inkMe.studio.BaseRequest
	40, // 31: inkMe.studio.RegisterResponse.response:type_name -> inkMe.studio.BaseResponse
	39, // 32: inkMe.studio.LoginRequest.request:type_name -> inkMe.studio.BaseRequest
	40, // 33: inkMe.studio.LoginResponse.response:type_name -> inkMe.studio.BaseResponse
	39, // 34: inkMe.studio.LogoutRequest.request:type_name -> inkMe.studio.BaseRequest
	40, // 35: inkMe.studio.LogoutResponse.response:type_name -> inkMe.studio.BaseResponse
	39, // 36: inkMe.studio.ValidateSessionRequest.request:type_name -> inkMe.studio.BaseRequest
	40, // 37: inkMe.studio.ValidateSessionResponse.response:type_name -> inkMe.studio.BaseResponse
	39, // 38: inkMe.studio.ChangeOwnPasswordRequest.request:type_name -> inkMe.studio.BaseRequest
	40, // 39: inkMe.studio.ChangeOwnPasswordResponse.response:type_name -> inkMe.studio.BaseResponse
	39, // 40: inkMe.studio.AdminResetUserPasswordRequest.request:type_name -> inkMe.studio.BaseRequest
	40, // 41: inkMe.studio.AdminResetUserPasswordResponse.response:type_name -> inkMe.studio.BaseResponse
	39, // 42: inkMe.studio.ChangePasswordRequest.request:type_name -> inkMe.studio.BaseRequest
	40, // 43: inkMe.studio.ChangePasswordResponse.response:type_name -> inkMe.studio.BaseResponse
	39, // 44: inkMe.studio.ChangeEmailRequest.request:type_name -> inkMe.studio.BaseRequest
	40, // 45: inkMe.studio.ChangeEmailResponse.response:type_name -> inkMe.studio.BaseResponse
	39, // 46: inkMe.studio.RefreshTokenRequest.request:type_name -> inkMe.studio.BaseRequest
	40, // 47: inkMe.studio.TokenResponse.response:type_name -> inkMe.studio.BaseResponse
	0,  // 48: inkMe.studio.StudioService.CreateStudio:input_type -> inkMe.studio.CreateStudioRequest
	2,  // 49: inkMe.studio.StudioService.UpdateStudio:input_type -> inkMe.studio.UpdateStudioRequest
	19, // 50: inkMe.studio.StudioService.ListStudios:input_type -> inkMe.studio.ListStudiosRequest
	4,  // 51: inkMe.studio.StudioService.AddStaffMember:input_type -> inkMe.studio.AddStaffMemberRequest
	6,  // 52: inkMe.studio.StudioService.UpdateStaffMember:input_type -> inkMe.studio.UpdateStaffMemberRequest
	8,  // 53: inkMe.studio.StudioService.RemoveStaffMember:input_type -> inkMe.studio.RemoveStaffMemberRequest
	10, // 54: inkMe.studio.StudioService.ListStaffMembers:input_type -> inkMe.studio.ListStaffMembersRequest
	12, // 55: inkMe.studio.StudioService.SetStaffPermissions:input_type -> inkMe.studio.SetStaffPermissionsRequest
	14, // 56: inkMe.studio.StudioService.GetStaffPermissions:input_type -> inkMe.studio.GetStaffPermissionsRequest
	21, // 57: inkMe.studio.AuthService.Register:input_type -> inkMe.studio.RegisterRequest
	23, // 58: inkMe.studio.AuthService.Login:input_type -> inkMe.studio.LoginRequest
	25, // 59: inkMe.studio.AuthService.Logout:input_type -> inkMe.studio.LogoutRequest
	33, // 60: inkMe.studio.AuthService.ChangePassword:input_type -> inkMe.studio.ChangePasswordRequest
	35, // 61: inkMe.studio.AuthService.ChangeEmail:input_type -> inkMe.studio.ChangeEmailRequest
	37, // 62: inkMe.studio.AuthService.RefreshToken:input_type -> inkMe.studio.RefreshTokenRequest
	27, // 63: inkMe.studio.AuthService.ValidateSession:input_type -> inkMe.studio.ValidateSessionRequest
	29, // 64: inkMe.studio.AuthService.ChangeOwnPassword:input_type -> inkMe.studio.ChangeOwnPasswordRequest
	31, // 65: inkMe.studio.AuthService.AdminResetUserPassword:input_type -> inkMe.studio.AdminResetUserPasswordRequest
	1,  // 66: inkMe.studio.StudioService.CreateStudio:output_type -> inkMe.studio.CreateStudioResponse
	3,  // 67: inkMe.studio.StudioService.UpdateStudio:output_type -> inkMe.studio.UpdateStudioResponse
	20, // 68: inkMe.studio.StudioService.ListStudios:output_type -> inkMe.studio.ListStudiosResponse
	5,  // 69: inkMe.studio.StudioService.AddStaffMember:output_type -> inkMe.studio.AddStaffMemberResponse
	7,  // 70: inkMe.studio.StudioService.UpdateStaffMember:output_type -> inkMe.studio.UpdateStaffMemberResponse
	9,  // 71: inkMe.studio.StudioService.RemoveStaffMember:output_type -> inkMe.studio.RemoveStaffMemberResponse
	11, // 72: inkMe.studio.StudioService.ListStaffMembers:output_type -> inkMe.studio.ListStaffMembersResponse
	13, // 73: inkMe.studio.StudioService.SetStaffPermissions:output_type -> inkMe.studio.SetStaffPermissionsResponse
	15, // 74: inkMe.studio.StudioService.GetStaffPermissions:output_type -> inkMe.studio.GetStaffPermissionsResponse
	22, // 75: inkMe.studio.AuthService.Register:output_type -> inkMe.studio.RegisterResponse
	24, // 76: inkMe.studio.AuthService.Login:output_type -> inkMe.studio.LoginResponse
	26, // 77: inkMe.studio.AuthService.Logout:output_type -> inkMe.studio.LogoutResponse
	34, // 78: inkMe.studio.AuthService.ChangePassword:output_type -> inkMe.studio.ChangePasswordResponse
	36, // 79: inkMe.studio.AuthService.ChangeEmail:output_type -> inkMe.studio.ChangeEmailResponse
	38, // 80: inkMe.studio.AuthService.RefreshToken:output_type -> inkMe.studio.TokenResponse
	28, // 81: inkMe.studio.AuthService.ValidateSession:output_type -> inkMe.studio.ValidateSessionResponse
	30, // 82: inkMe.studio.AuthService.ChangeOwnPassword:output_type -> inkMe.studio.ChangeOwnPasswordResponse
	32, // 83: inkMe.studio.AuthService.AdminResetUserPassword:output_type -> inkMe.studio.AdminResetUserPasswordResponse
	66, // [66:84] is the sub-list for method output_type
	48, // [48:66] is the sub-list for method input_type
	48, // [48:48] is the sub-list for extension type_name
	48, // [48:48] is the sub-list for extension extendee
	0,  // [0:48] is the sub-list for field type_name
}

func init() { file_studio_proto_init() }
func file_studio_proto_init() {
	if File_studio_proto != nil {
		return
	}
	type x struct{}
	out := protoimpl.TypeBuilder{
		File: protoimpl.DescBuilder{
			GoPackagePath: reflect.TypeOf(x{}).PkgPath(),
			RawDescriptor: unsafe.Slice(unsafe.StringData(file_studio_proto_rawDesc), len(file_studio_proto_rawDesc)),
			NumEnums:      0,
			NumMessages:   41,
			NumExtensions: 0,
			NumServices:   2,
		},
		GoTypes:           file_studio_proto_goTypes,
		DependencyIndexes: file_studio_proto_depIdxs,
		MessageInfos:      file_studio_proto_msgTypes,
	}.Build()
	File_studio_proto = out.File
	file_studio_proto_goTypes = nil
	file_studio_proto_depIdxs = nil
}
